...
public class PersonMapper extends AbstractMapper<PersonEntity, Person> {
    ...
    public Person fromEntity(PersonEntity entity) {
        return Person.builder().id(entity.getId()).name(entity.getName()).isEmployed(entity.isEmployed()).build();
    }

    @Override
    public PersonEntity toEntity(Person person) {
        return PersonEntity.builder().id(person.getId()).name(person.getName()).isEmployed(person.getIsEmployed()).build();
    }
